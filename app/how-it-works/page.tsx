...
import {
  HiwStepIcon1, HiwStepIcon2, HiwStepIcon3, HiwStepIcon4,
  HiwSection1, HiwSection2, HiwSection3, HiwSection4
} from "../components/Illustrations";

export const metadata = { title: "How It Works — BitStats" };

const steps = [
  { num:"01", label:"Foundation", color:"var(--blue)",   bg:"#F0F5FF",  Icon: HiwStepIcon1, Big: HiwSection1, title:"Create your BNS name as identity root",        body:"Your BNS name becomes the anchor for everything that follows. It's where your identity begins — clean, permanent, and simple. Register once on Stacks and you own it cryptographically. No company can revoke it, no server can lose it." },
  { num:"02", label:"Issuance",   color:"var(--green)",  bg:"#F0FBF8",  Icon: HiwStepIcon2, Big: HiwSection2, title:"Credentials anchored on the blockchain",        body:"Every credential lives onchain, auditable and permanent. Trust comes from the ledger itself, not from intermediaries. You own what's issued to you — tied directly to your BNS name with a cryptographic hash." },
  { num:"03", label:"Privacy",    color:"#C47A00",       bg:"#FFFBF5",  Icon: HiwStepIcon3, Big: HiwSection3, title:"Prove what matters, stay private",              body:"Zero-knowledge proofs let you demonstrate eligibility without exposing the details underneath. You show you meet the requirement. Nothing more gets revealed — your passport number, income, and personal details stay encrypted on your device." },
  { num:"04", label:"Verification",color:"var(--text-3)",bg:"#F4F6FA", Icon: HiwStepIcon4, Big: HiwSection4, title:"Instant verification for any dapp",             body:"Any application can call bil-verifier.clar and get a boolean response in under 2 seconds. The verifier knows you meet the requirement — nothing more. No raw data is ever transmitted." },
];
      ...
      <main style={{ paddingTop:68 }}>
        ...
        <section style={{ padding:"72px 5%", background:"var(--bg2)", borderBottom:"1px solid var(--border)" }}>
          <div className="container">
            ...
            <p style={{ fontSize:18, color:"var(--text-3)", maxWidth:560, lineHeight:1.7 }}>Four steps from identity to private verification — all anchored to Bitcoin.</p>
          </div>
        </section>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:24 }} className="steps-grid">
              {steps.map(s => (
                <div key={s.num} style={{ border:"1px solid var(--border)", borderRadius:16, overflow:"hidden" }}>
                  <div style={{ background:s.bg, aspectRatio:"3/2", overflow:"hidden" }}>
                    <s.Icon />
                  </div>
        ...
        {steps.map((s, i) => (
          <section key={s.num} id={`step-${i+1}`} style={{ padding:"80px 5%", background: i%2===1 ? "var(--bg2)" : "white", borderTop:"1px solid var(--border)" }}>
            <div style={{ maxWidth:1100, margin:"0 auto", display:"grid", gridTemplateColumns:"1fr 1fr", gap:80, alignItems:"center" }} className="scroll-inner">
              ...
              <div style={{ order: i%2===1 ? 1 : 2, borderRadius:16, aspectRatio:"4/3", overflow:"hidden", background:s.bg }}>
                <s.Big />
              </div>
            </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:0, border:"1px solid var(--border)", borderRadius:16, overflow:"hidden" }} className="zk-grid">
              {zkSteps.map((z, i) => (
                <div key={z.num} style={{ padding:28, background:"white", borderRight: i<3 ? "1px solid var(--border)" : "none", position:"relative" }}>
                  <div style={{ width:36, height:36, borderRadius:"50%", background:z.bg, color:z.color, display:"flex", alignItems:"center", justifyContent:"center", fontWeight:800, fontSize:14, marginBottom:16 }}>{z.num}</div>
