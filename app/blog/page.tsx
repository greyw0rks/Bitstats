...
const posts = [
  { slug: "post", cat: "Identity", catColor: "var(--blue)", catBg: "var(--blue-light)", icon: "🪪", bg: "var(--blue-light)", title: "Why BNS is the best identity root for Bitcoin", excerpt: "Your digital identity is not yours. Right now, fragments of who you are live on servers controlled by companies whose interests diverge from yours the moment a breach becomes profitable.", author: "BNS Team", date: "Mar 10, 2026", read: "4 min" },
  { slug: "post", cat: "Credentials", catColor: "var(--green)", catBg: "var(--green-light)", icon: "🔐", bg: "var(--green-light)", title: "Comparing Groth16, PLONK and STARKs for identity", excerpt: "Not all zero-knowledge proof systems are created equal. We examine the tradeoffs between the three leading approaches and why Groth16 wins for identity use cases.", author: "ZK Team", date: "Mar 5, 2026", read: "8 min" },
  { slug: "post", cat: "Technology", catColor: "#C47A00", catBg: "var(--amber-light)", icon: "⚙️", bg: "var(--amber-light)", title: "How Stacks Nakamoto makes BitStats faster", excerpt: "The Nakamoto upgrade changes everything for Stacks-based applications. Here's how sub-second block times and Bitcoin finality improve the BitStats verification flow.", author: "Core Team", date: "Feb 28, 2026", read: "5 min" },
  { slug: "post", cat: "Updates", catColor: "var(--text-3)", catBg: "var(--bg3)", icon: "📢", bg: "var(--bg3)", title: "DeGrants Cohort 4 — our roadmap", excerpt: "We've applied to the Stacks DeGrants program for Cohort 4. Here's what we're building, our milestone plan, and how the grant will accelerate our roadmap.", author: "Team BitStats", date: "Feb 20, 2026", read: "6 min" },
  { slug: "post", cat: "Identity", catColor: "var(--blue)", catBg: "var(--blue-light)", icon: "🏛️", bg: "var(--blue-light)", title: "The case for Bitcoin-native identity", excerpt: "Every major blockchain ecosystem has an identity layer — except Bitcoin. We argue why Bitcoin-native identity through BNS is not just possible but essential for the next wave of adoption.", author: "BNS Team", date: "Feb 14, 2026", read: "7 min" },
];
      ...
      <main style={{ paddingTop: 68 }}>
        ...
        <section style={{ padding: "72px 5%", background: "var(--bg2)", borderBottom: "1px solid var(--border)" }}>
          <div className="container">
            ...
            <p style={{ fontSize: 18, color: "var(--text-3)", maxWidth: 540, lineHeight: 1.7 }}>
              Zero-knowledge proofs, BNS, Stacks, and the future of self-sovereign identity.
            </p>
          </div>
        </section>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
              {posts.map((p, i) => (
                <Link key={i} href={`/blog/${p.slug}`} style={{ display: "grid", gridTemplateColumns: "120px 1fr", gap: 24, padding: "28px 0", borderBottom: "1px solid var(--border)", alignItems: "center", textDecoration: "none" }} className="post-row">
                  <div style={{ aspectRatio: "4/3", borderRadius: 10, background: p.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 36 }}>{p.icon}</div>
                  <div>
        ...
        <section className="section" style={{ background: "var(--bg2)" }}>
          <div className="container">
            <div style={{ marginBottom: 32 }}>
              <h2 style={{ fontSize: "clamp(1.4rem,2.5vw,2rem)" }}>More from our writers</h2>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 20 }} className="feat-grid">
              {featured.map((p, i) => (
                <Link key={i} href={`/blog/${p.slug}`} style={{ border: "1px solid var(--border)", borderRadius: 14, overflow: "hidden", background: "white", textDecoration: "none", display: "block" }}>
                  <div style={{ aspectRatio: "16/9", background: p.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 32 }}>{p.icon}</div>
                  <div style={{ padding: "16px 18px" }}>
                    ...
                    <div style={{ fontSize: 12, color: "var(--text-4)", marginTop: 8 }}>{p.author} · {p.read}</div>
                  </div>
                </Link>
              ))}
            </div>
